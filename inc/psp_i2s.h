...
#ifdef ENABLE_RECORD
// setup record processing path for right channel
//#define ENABLE_STEREO_RECORD

// set the rx sample rate (ADC) - only one sample rate should be defined
#define SAMPLE_RATE_RX_48kHz
//#define SAMPLE_RATE_RX_16kHz
#endif

// set the tx sample rate (DAC) - only one sample rate should be defined
#define SAMPLE_RATE_TX_48kHz
//#define SAMPLE_RATE_TX_44_1kHz
//#define SAMPLE_RATE_TX_32kHz
//#define SAMPLE_RATE_TX_16kHz

/* Size of codec Tx ping or pong buffer in msec. */
#define TXBUFF_SZ_MSEC              (1)
//#define TXBUFF_SZ_MSEC              (2)
//#define TXBUFF_SZ_MSEC              (10)

/* Size of codec Tx ping or pong buffer in DAC samples, mono */
#define TXBUFF_SZ_DACSAMPS_48KHZ    (TXBUFF_SZ_MSEC*48)
#define TXBUFF_SZ_DACSAMPS_44KHZ    (TXBUFF_SZ_MSEC*44)
#define TXBUFF_SZ_DACSAMPS_32KHZ    (TXBUFF_SZ_MSEC*32)
#define TXBUFF_SZ_DACSAMPS_16KHZ    (TXBUFF_SZ_MSEC*16)
/* Size of codec Tx ping or pong buffer in I2S samples, mono */
#define TXBUFF_SZ_I2SSAMPS_48KHZ    (TXBUFF_SZ_DACSAMPS_48KHZ)
#define TXBUFF_SZ_I2SSAMPS_44KHZ    (TXBUFF_SZ_DACSAMPS_44KHZ)
#define TXBUFF_SZ_I2SSAMPS_32KHZ    (TXBUFF_SZ_DACSAMPS_32KHZ)
#define TXBUFF_SZ_I2SSAMPS_16KHZ    (TXBUFF_SZ_DACSAMPS_16KHZ)

#ifdef SAMPLE_RATE_TX_48kHz
#define MAX_TXBUFF_SZ_DACSAMPS      (TXBUFF_SZ_DACSAMPS_48KHZ)
#define MAX_TXBUFF_SZ_I2SSAMPS      (MAX_TXBUFF_SZ_DACSAMPS)

#elif defined(SAMPLE_RATE_TX_44_1kHz)
#define MAX_TXBUFF_SZ_DACSAMPS      (TXBUFF_SZ_I2SSAMPS_44KHZ)
#define MAX_TXBUFF_SZ_I2SSAMPS      (MAX_TXBUFF_SZ_DACSAMPS)

#elif defined (SAMPLE_RATE_TX_32kHz)
#define MAX_TXBUFF_SZ_DACSAMPS      (TXBUFF_SZ_I2SSAMPS_32KHZ)
#define MAX_TXBUFF_SZ_I2SSAMPS      (MAX_TXBUFF_SZ_DACSAMPS)

#elif defined(SAMPLE_RATE_TX_16kHz)
#define MAX_TXBUFF_SZ_DACSAMPS      (TXBUFF_SZ_I2SSAMPS_16KHZ)
#define MAX_TXBUFF_SZ_I2SSAMPS      (MAX_TXBUFF_SZ_DACSAMPS)
#endif

/* I2S Tx buffer size */
#define MAX_I2S_TXBUFF_SZ           (2*MAX_TXBUFF_SZ_I2SSAMPS) /* 2x for stereo */

#define CODEC_OUTPUT_BUFFER_SIZE    (6*MAX_I2S_TXBUFF_SZ)

// set the I2S rx sample rate - only one sample rate should be defined
// NOTE: currently set to DAC_FS in codec configuration
#define SAMPLE_RATE_I2S_48kHz
//#define SAMPLE_RATE_I2S_16kHz

/* Size of codec Rx ping or pong buffer in msec. */
#define RXBUFF_SZ_MSEC              (1)
//#define RXBUFF_SZ_MSEC              (2)
//#define RXBUFF_SZ_MSEC              (10)

#if defined(SAMPLE_RATE_RX_16kHz) && defined(SAMPLE_RATE_I2S_48kHz) /* ADC: 16kHz, I2S: 48kHz */
...
#define RXBUFF_SZ_ADCSAMPS          (RXBUFF_SZ_MSEC*16) /* size of codec Rx ping or pong buffer in ADC samples */
#define RXBUFF_SZ_I2SSAMPS          (RXBUFF_SZ_ADCSAMPS*3) /* size of codec Rx ping or pong buffer in I2S samples */

#elif defined(SAMPLE_RATE_RX_16kHz) /* ADC: 16kHz, I2S: 16kHz */
// since we need datapack to be disabled on I2S tx, we need it disabled on I2S rx therefore
// we get 2 words per DMA transfer so the offset into DMA buffers has to be twice as big
#define RXBUFF_SZ_ADCSAMPS          (RXBUFF_SZ_MSEC*16) /* size of codec Rx ping or pong buffer in ADC samples */
#define RXBUFF_SZ_I2SSAMPS          (RXBUFF_SZ_ADCSAMPS) /* size of codec Rx ping or pong buffer in I2S samples */

#else /* ADC: 48kHz, I2S: 48kHz */
// since we need datapack to be disabled on I2S tx, we need it disabled on I2S rx therefore
// we get 2 words per DMA transfer so the offset into DMA buffers has to be twice as big
#define RXBUFF_SZ_ADCSAMPS          (RXBUFF_SZ_MSEC*48) /* size of codec Rx ping or pong buffer in ADC samples */
#define RXBUFF_SZ_I2SSAMPS          (RXBUFF_SZ_ADCSAMPS) /* size of codec Rx ping or pong buffer in I2S samples */
...
#define CODEC_INPUT_BUFFER_FRAME_SZ     (RXBUFF_SZ_ADCSAMPS)
#else
#define CODEC_INPUT_BUFFER_FRAME_SZ     (2*RXBUFF_SZ_ADCSAMPS) /* 2x for stereo */
#endif

#define CODEC_INPUT_BUFFER_SIZE         (10*CODEC_INPUT_BUFFER_FRAME_SZ)  /* for smaller input frame size, e.g. 1 msec. */
//#define CODEC_INPUT_BUFFER_SIZE         (2*CODEC_INPUT_BUFFER_FRAME_SZ) /* for larger input frame size, e.g. 10 msec. */
